...
### Jennifer categories
#Survival curve calculation
fit<- survfit(Surv(ProgressionFree_Length, ProgressionStatus) ~ Aggression.category.detailed, data = PFS_Jennifer_Well_DiffMalignant)
print(fit)

# Most basic plot
ggsurvplot(fit, data=PFS_Jennifer_Well_DiffMalignant)

# More involved, customized plot
PFS_Plot <- ggsurvplot(fit,
                       data=PFS_Jennifer_Well_DiffMalignant,
                       #pval = TRUE, pval.method = TRUE, conf.int = FALSE,
                       risk.table = TRUE, 
                       risk.table.col = "strata", 
...
p2 = PFS_Plot$table
plotp = cowplot::plot_grid(p1,p2,align = "v",axis = "b", ncol =1, rel_heights = c(3,1))
ggsave("outputs/well_diff/PFS/22-0909_Well_Diff_Jennifer_categories_PFS.png", 
       plot = plotp, 
       width = 7, height = 7,
       dpi = 600)
