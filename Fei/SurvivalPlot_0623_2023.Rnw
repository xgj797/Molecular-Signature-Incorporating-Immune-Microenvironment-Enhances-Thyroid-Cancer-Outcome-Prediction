...
p.text<-function(p) ifelse(p>0.99,'>0.99',
                           ifelse(p<0.0001,'p<0.001',paste0('p=',round(p,3))))
...
\item Variables:'TERT mutation','TP53 mutation','PIK3CA mutation','BRS.PN (BRAF- or RAS-like)', 'ATS.PN (MAP score: positive vs. negative)'
\begin{itemize}
   ...
   \item Cohort subgroups
       \begin{itemize}
          \item A: Malignant (Differentiated+Transformed)
         \item B: Malignant well-differentiated (Differentiated)
       \end{itemize}
    \end{itemize}
...
\subsection{Malignant (Differentiated+Transformed)}
<<include=FALSE>>=
shortname<-'malignant'
...
predictor.var<-c( "TERT.mutation","TP53.mutation" , "PIK3CA.mutation" ,"BRS.PN","ATS.PN")
legend.title.v<-c('Mutation','Mutation','Mutation','BRAF- or RAS-like','MAP score')
group.name.v<-c( "TERT promoter","TP53" , "PIK3CA" ,'BRAF- or RAS-like','MAP score')
datin=subset(d0.a.clean,select=c(predictor.var,survival.var)) ####manual
event<-'event.PFS'
y.lab<-"Progression Free Survival"

#relevel
datin$TERT.mutation<-factor(datin$TERT.mutation,levels=c('yes','no'),labels=c('Yes','No'))
datin$TP53.mutation<-factor(datin$TP53.mutation,levels=c('yes','no'),labels=c('Yes','No'))
datin$PIK3CA.mutation<-factor(datin$PIK3CA.mutation,levels=c('yes','no'),labels=c('Yes','No'))
datin$BRS.PN<-factor(datin$BRS.PN,levels=c('positive','negative'),labels=c('BRAF-like','RAS-like'))
datin$ATS.PN<-factor(datin$ATS.PN,levels=c('positive','negative'),labels=c('Positive','Negative'))


 block.out<-NULL

 ll<-length(predictor.var)
 fig.name.v<-c('Figure 1D','Figure 1E','Figure 1F','Figure S2C','Figure 5B left panel')
  for (i in 1:ll){
  group<-predictor.var[i]
  legend.title<-legend.title.v[i]
  fig.name<-fig.name.v[i]
  group.name<-group.name.v[i]
  ...
  if (i %in% c(1,2,3,4)) palette.value=my.color[c(2:1)]
  if (i %in% 4) palette.value=my.color[c(1:2)]
  if (i==5) palette.value=c('#BA68C8' ,'#4A148C')
...
\subsection{Malignant well-differentiated (Differentiated)}
<<include=FALSE>>=
...
predictor.var<-c( "BRS.PN","ATS.PN")
legend.title.v<-c('BRAF- or RAS-like','MAP score')
group.name.v<-c('BRAF- or RAS-like','MAP score')


datin=subset(d0.b.clean,select=c(predictor.var,survival.var)) 
event<-'event.PFS'
...
datin$ATS.PN<-factor(datin$ATS.PN,levels=c('positive','negative'),labels=c('Positive','Negative'))
 ...
 fig.name.v<-c('Figure S2B','Figure 5B right panel')
 for (i in 1:2){
  group<-predictor.var[i]
  legend.title<-legend.title.v[i]
  fig.name<-fig.name.v[i]
  group.name<-group.name.v[i]
    ...
    if (i==1) palette.value=my.color[c(1:2)]
   if (i==2) palette.value=c('#BA68C8' ,'#4A148C')
...
predictor.var<-c( "TERT.mutation","TP53.mutation" , "PIK3CA.mutation","ATS.PN"   )
legend.title.v<-c('Mutation','Mutation','Mutation','MAP score')
group.name.v<-c( "TERT promoter","TP53" , "PIK3CA",'MAP score' )
fig.name.v<-c('Figure S1A','Figure S1A','Figure S1A','Figure S8A')
...
#	Each marker+gender+age
 ...
 palette.value=my.color[c(2:1)]
  ...
  group.name<-group.name.v[i]
    ...
    pval.xy=c(1,0.2)
    pval.method.xy=c(1,0.1)
    if (i==4) palette.value=c('#BA68C8' ,'#4A148C')
    block.out = c(block.out, knit_child('Child.Cox.block.Rnw'))
