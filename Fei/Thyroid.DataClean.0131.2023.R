...
data.cleaning<-function(d0,clic.var,survival.var,predictor.var,poor.var){
  ...
  d0<-within(d0,{
    ...
    Cancer.associated.fibroblast_EPIC_high<-factor(Cancer.associated.fibroblast_EPIC_high,levels=c(0,1),labels=c('no','yes'))
    Cancer.associated.fibroblast_MCPCOUNTER_high<-factor(Cancer.associated.fibroblast_MCPCOUNTER_high,levels=c(0,1),labels=c('no','yes'))
    
    BRS.PN<-ifelse(is.na(BRS),NA,ifelse(BRS>=0,'positive','negative'))
    BRS.PN<-factor(BRS.PN,levels=c('negative','positive'))
   ...
   # label(RAS.family.mutation)<-'RAS family mutation'
    label(RAS.mutation)<-'RAS mutation'
    label(TERT.TP53.PIK3CA.mut)<-'Mutation in TERT, TP53, PIK3CA'
    label(BRS)<-'BRS'
    #label(BRAFB)<-'BRAFB'
   # label(BRAFPoorOutcome)<-'BRAF Poor Outcome'
    label(BRS.PN)<-'BRS'
    label(ATS)<-'ATS'
    label(ATS.PN)<-'ATS'
   # label(BRAFB.PN)<-'BRAFB'
   # label(BRAFPoorOutcome.PN)<-'BRAF Poor Outcome'
    label(BRAFPoorOutcome_526)<-'BRAFPoorOutcome 526'
    #label(BRAFPoorOutcome_526.PN)<-'BRAFPoorOutcome 526'
    #label(BRAFPoorOutcome_526_PTCandIFVPTConly)<-'BRAFPoorOutcome 526 PTCandIFVPTConly'
    #label(BRAFPoorOutcome_526_PTCandIFVPTConly.PN)<-'BRAFPoor526PTC.IFVPTC'
    ...
    label(Cancer.associated.fibroblast_EPIC_high)<-'Cancer associated fibroblast EPIC high'
    label(Cancer.associated.fibroblast_MCPCOUNTER_high)<-'Cancer associated fibroblast MCPCOUNTER high'
...
d.data.name<-data.frame(dataset=c('1','2','3','4'), #use
                        names=c('all local malignant','local malignant well-diff',
                                'future aggression','future aggression on 3 mut'))

file.name<-'VUMC.cohort.GX_1-31-23_for-Fei.xlsx' #data set 1,2,3,4
...
# All local malignant (excel sheet 1) ------------------------------------
d0.a<-openxlsx::read.xlsx((paste(path.in,file.name,sep='')),detectDates=TRUE,
                        sheet='1 - local malignant',na.strings = c("NA","NaN"," "))
...
# local malignant well-diff (excel sheet 2) -----------
d0.b<-openxlsx::read.xlsx((paste(path.in,file.name,sep='')),detectDates=TRUE,
                          sheet='2 - well diff',na.strings = c("NA","NaN"," "))
...
# PTC/IFVPTC (excel sheet 3) ---------------------------------------------------
 d0.c<-openxlsx::read.xlsx((paste(path.in,file.name,sep='')),detectDates=TRUE,
                          sheet='3 - future aggression',na.strings = c("NA","NaN"," "))
d0<-d0.c
t.names<-names(d0)
duplicate.var<-names(which(table(t.names)>1))
which.no<-which(t.names==duplicate.var) #duplicate name column
...
#before aggression (excel sheet 4) ----------------------
d0.d<-openxlsx::read.xlsx((paste(path.in,file.name,sep='')),detectDates=TRUE,
                          sheet='4 - future aggression no 3 mut',na.strings = c("NA","NaN"," "))
d0<-d0.d
t.names<-names(d0)
duplicate.var<-names(which(table(t.names)>1))
which.no<-which(t.names==duplicate.var) #duplicate name column
...
save(d0.a.clean,d0.b.clean,d0.c.clean,d0.d.clean,
    clic.var, survival.var,predictor.var,
    d.data.name,
     file=paste0(path.in,'Thyroid.0131.2023.Rdata'))
