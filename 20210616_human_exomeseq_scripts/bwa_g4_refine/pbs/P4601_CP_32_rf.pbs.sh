...
gatk --java-options "-Xmx40G" \
  MarkDuplicates \
  --INPUT P4601_CP_32.sortedByCoord.bam \
  --OUTPUT P4601_CP_32.rmdup.bam \
  --METRICS_FILE P4601_CP_32.rmdup.bam.metrics \
...
status=$?
if [[ $status -ne 0 ]]; then
  touch P4601_CP_32.MarkDuplicates.failed
  rm -f P4601_CP_32.rmdup.bam P4601_CP_32.rmdup.bam.metrics P4601_CP_32.rmdup.bai
else
  echo BaseRecalibrator=`date` 
  gatk --java-options "-Xmx40G" \
    BaseRecalibrator \
    -R /data/cqs/references/broad/hg38/v0/bwa_index_0.7.17/Homo_sapiens_assembly38.fasta \
    -I P4601_CP_32.rmdup.bam \
    --use-original-qualities  \
     --known-sites /data/cqs/references/broad/hg38/v0/Mills_and_1000G_gold_standard.indels.hg38.vcf.gz \
    -O P4601_CP_32.rmdup.recal_data.csv  

  status=$?
  if [[ $status -ne 0 ]]; then
    touch P4601_CP_32.BaseRecalibrator.failed
  else
    echo ApplyBQSR=`date`
    gatk --java-options "-Xmx40G" \
      ApplyBQSR \
      -R /data/cqs/references/broad/hg38/v0/bwa_index_0.7.17/Homo_sapiens_assembly38.fasta \
      -I P4601_CP_32.rmdup.bam \
      -O P4601_CP_32.rmdup.recal.bam \
      -bqsr P4601_CP_32.rmdup.recal_data.csv \
    ...
    status=$?
    if [[ $status -ne 0 ]]; then
      touch P4601_CP_32.ApplyBQSR.failed
    else
      touch P4601_CP_32.succeed
      ln P4601_CP_32.rmdup.recal.bai P4601_CP_32.rmdup.recal.bam.bai

      echo flagstat = `date` 
      samtools flagstat P4601_CP_32.rmdup.recal.bam > P4601_CP_32.rmdup.recal.bam.stat 

      echo flagstat = `date` 
      samtools idxstats P4601_CP_32.rmdup.recal.bam > P4601_CP_32.rmdup.recal.bam.chromosome.count 

      rm /scratch/weissvl/shengq2/20210616_human_exomeseq/bwa_g4_refine/result/P4601_CP_32.*.failed
      rm  P4601_CP_32.rmdup.bam P4601_CP_32.rmdup.bai
    fi
  fi
fi
