...
gatk --java-options "-Xmx40G" \
  MarkDuplicates \
  --INPUT P6121_CP_31.sortedByCoord.bam \
  --OUTPUT P6121_CP_31.rmdup.bam \
  --METRICS_FILE P6121_CP_31.rmdup.bam.metrics \
...
status=$?
if [[ $status -ne 0 ]]; then
  touch P6121_CP_31.MarkDuplicates.failed
  rm -f P6121_CP_31.rmdup.bam P6121_CP_31.rmdup.bam.metrics P6121_CP_31.rmdup.bai
else
  echo BaseRecalibrator=`date` 
  gatk --java-options "-Xmx40G" \
    BaseRecalibrator \
    -R /data/cqs/references/broad/hg38/v0/bwa_index_0.7.17/Homo_sapiens_assembly38.fasta \
    -I P6121_CP_31.rmdup.bam \
    --use-original-qualities  \
     --known-sites /data/cqs/references/broad/hg38/v0/Mills_and_1000G_gold_standard.indels.hg38.vcf.gz \
    -O P6121_CP_31.rmdup.recal_data.csv  

  status=$?
  if [[ $status -ne 0 ]]; then
    touch P6121_CP_31.BaseRecalibrator.failed
  else
    echo ApplyBQSR=`date`
    gatk --java-options "-Xmx40G" \
      ApplyBQSR \
      -R /data/cqs/references/broad/hg38/v0/bwa_index_0.7.17/Homo_sapiens_assembly38.fasta \
      -I P6121_CP_31.rmdup.bam \
      -O P6121_CP_31.rmdup.recal.bam \
      -bqsr P6121_CP_31.rmdup.recal_data.csv \
    ...
    status=$?
    if [[ $status -ne 0 ]]; then
      touch P6121_CP_31.ApplyBQSR.failed
    else
      touch P6121_CP_31.succeed
      ln P6121_CP_31.rmdup.recal.bai P6121_CP_31.rmdup.recal.bam.bai

      echo flagstat = `date` 
      samtools flagstat P6121_CP_31.rmdup.recal.bam > P6121_CP_31.rmdup.recal.bam.stat 

      echo flagstat = `date` 
      samtools idxstats P6121_CP_31.rmdup.recal.bam > P6121_CP_31.rmdup.recal.bam.chromosome.count 

      rm /scratch/weissvl/shengq2/20210616_human_exomeseq/bwa_g4_refine/result/P6121_CP_31.*.failed
      rm  P6121_CP_31.rmdup.bam P6121_CP_31.rmdup.bai
    fi
  fi
fi
