...
gatk --java-options "-Xmx40G" CollectReadCounts  \
  ...
  --input /scratch/weissvl/shengq2/20210616_human_exomeseq/bwa_g4_refine/result/P4271_CP_26.rmdup.recal.bam \
  --reference /data/cqs/references/broad/hg38/v0/Homo_sapiens_assembly38.fasta \
  --format HDF5 \
  --interval-merging-rule OVERLAPPING_ONLY \
  --output P4271_CP_26.count.hdf5

status=$?
if [[ $status -ne 0 ]]; then
  touch P4271_CP_26.failed
  rm -f P4271_CP_26.count.hdf5
else
