...
gatk --java-options "-Xmx40G" CollectReadCounts  \
  ...
  --input /scratch/weissvl/shengq2/20210616_human_exomeseq/bwa_g4_refine/result/P1809_AC_020.rmdup.recal.bam \
  --reference /data/cqs/references/broad/hg38/v0/Homo_sapiens_assembly38.fasta \
  --format HDF5 \
  --interval-merging-rule OVERLAPPING_ONLY \
  --output P1809_AC_020.count.hdf5

status=$?
if [[ $status -ne 0 ]]; then
  touch P1809_AC_020.failed
  rm -f P1809_AC_020.count.hdf5
else
