...
for i in {1..5}; do 
  echo iteration $i ...
  cp -fL /data/h_vivian_weiss/1660/1660-AC-11-GGCTACAT-TCTTTCCC_S37_R1_001.fastq.gz 1660-AC-11-GGCTACAT-TCTTTCCC_S37_R1_001.fastq.gz
...
for i in {1..5}; do 
  echo iteration $i ...
  cp -fL /data/h_vivian_weiss/1660/1660-AC-11-GGCTACAT-TCTTTCCC_S37_R2_001.fastq.gz 1660-AC-11-GGCTACAT-TCTTTCCC_S37_R2_001.fastq.gz
...
cutadapt -j 2 -q 20 -a AGATCGGAAGAG -A AGATCGGAAGAG -n 3 --trim-n -a "A{50}" -a "T{50}" -a "G{50}" -a "C{50}" -A "A{50}" -A "T{50}" -A "G{50}" -A "C{50}" -o X1660AC11_clipped.1.fastq.gz -p X1660AC11_clipped.2.fastq.gz  -m 30  --too-short-output=X1660AC11_clipped.1.fastq.short.gz --too-short-paired-output=X1660AC11_clipped.2.fastq.short.gz 1660-AC-11-GGCTACAT-TCTTTCCC_S37_R1_001.fastq.gz 1660-AC-11-GGCTACAT-TCTTTCCC_S37_R2_001.fastq.gz
status=$?
if [[ $status -eq 0 ]]; then
  touch X1660AC11.succeed
  md5sum X1660AC11_clipped.1.fastq.gz > X1660AC11_clipped.1.fastq.gz.md5
  md5sum X1660AC11_clipped.2.fastq.gz > X1660AC11_clipped.2.fastq.gz.md5
else
  rm X1660AC11_clipped.1.fastq.gz X1660AC11_clipped.2.fastq.gz
  touch X1660AC11.failed
fi


rm 1660-AC-11-GGCTACAT-TCTTTCCC_S37_R1_001.fastq.gz 1660-AC-11-GGCTACAT-TCTTTCCC_S37_R2_001.fastq.gz

cutadapt --version 2>&1 | awk '{print "Cutadapt,v"$1}' > X1660AC11.version
